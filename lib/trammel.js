...
module.exports = (dir, options, callback) => {
    ...
    if (!callback) return new Promise(next)

    next();

    function next(resolve, reject) {

        let promise = (typeof resolve === 'function');
        ...
        emitter.on('error', (err) => {
            promise ? reject(err) : callback(err, null);
        });
        
        emitter.on('end', () => {
            if (type === 'raw')
                return promise ? resolve(total) : callback(null, total);
            
            promise ? resolve(format.size(total)) : callback(null, format.size(total));
        });
